...
NEVER USE IT - OR AT LEAST BEWARE / JDN
    NEVER USE IT - OR AT LEAST BEWARE / JDN
    NEVER USE IT - OR AT LEAST BEWARE / JDN
    NEVER USE IT - OR AT LEAST BEWARE / JDN
    NEVER USE IT - OR AT LEAST BEWARE / JDN
...
int k_init(int nrTask, int nrSem, int nrMsg)
{
    ...
    if ((task_pool == NULL) || (sem_pool == NULL)
        || (send_pool == NULL)) {
        k_err_cnt++;
        goto leave;
    }
...
// if timer 0 we will alwasy run 1 msec !!!! and run org millis om nehalf of original Arduino interrupt
    // rethink more thna once if you want to use timer0 - the include "millis" code in the ISR (as dne above)
