...
*/
int k_ticksize(void) { return k_tick_size; }
...
struct k_t *k_crt_sem(int init_val, int maxvalue) {
  ...
  if ((maxvalue < init_val) || (MAX_SEM_VAL < maxvalue) || (init_val < 0) ||
      (maxvalue < 0)) {
    goto badexit;
  }
...
int k_start() {
  ...
  https://wolles-elektronikkiste.de/en/timer-and-pwm-part-1-8-bit-timer0-2
     ...
     0 1 0 clk T2S /8 (From prescaler)      16M/2 = 2000000 intr/sec at 1
  downcount 0 1 1 clk T2S /32 (From prescaler)      500000 intr/sec ... 1 0 0
  clk T2S /64 (From prescaler)      250000 1 0 1 clk T2S /128 (From prescaler)
  125000 1 1 0 clk T 2 S /256 (From prescaler)    62500 1 1 1 clk T 2 S /1024
  (From prescaler)   16M/1024=15625  eq 15.625 count down for 1 millisec so 255
  counts ~= 80.32 milli sec timer
...
#else
        // should be default ASSR &= ~(1 << AS2);   // Select clock source:
        // internal
  ...
  TCCRxB = PRESCALE; //(1<< CS22) | (1 << CS21)| (1 << CS20);//0x05; //  CS2 CS1
                     //CS0 set so  prescaler = 1024
...
void k_set_coop_multitask(unsigned char onn) { k_coopFlag = onn; }
